...
- [ ] **1.7 `build_assoc` / `put_assoc` helpers**  
...
- [ ] **2.1 Compile-time schema verification (`necto_verify`)**  
  При компилация, ако `NECTO_VERIFY=1` и има DB connection, проверяваме:
  - Съществува ли таблицата?
  - Съвпадат ли колоните по име и тип?
  - Съществуват ли foreign key constraint-ите?
  Грешката е **compile-time error**, не runtime exception.
...
- [ ] **2.4 Static query analysis**  
  Макрос `verifyQuery` който при compile-time проверява дали заявката е синтактично валиден SQL (през PostgreSQL `EXPLAIN` без execution).
- [ ] **2.5 Zero-overhead benchmark suite**  
  Benchmark срещу: ръчен SQL (db_postgres), Norm, Ecto (Elixir), Avram (Crystal). Да докажем че макросите не костват performance.
- [ ] **2.6 Compiled query cache**  
  За статични заявки (без runtime params), генерираме SQL string на compile-time и го пазим като константа.
