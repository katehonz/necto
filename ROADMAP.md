...
- [x] **3.1 Prepared statement cache** (per-connection, per-adapter) с hit/miss метрики
- [x] **3.2 Connection pool metrics** — wait time, active connections, queue depth (Prometheus формат)
- [x] **3.3 Query timeout и slow query log** — `statement_timeout` + slow query counter
- [x] **3.4 Read replica support** — `readAdapter`, `getReadConn`/`getWriteConn` routing
- [x] **3.5 Transaction savepoints** — `repo.savepoint(name):`, `repo.rollbackTo(name)`
...
2. **Multi-database.** PostgreSQL е primary, но MariaDB и SQLite са поддържани.
3. **Минимален код.** Nim макросите са мощни — ползваме ги за кратък, четим код.
4. **No breaking changes след 0.4.0.** До тогава API-то може да се движи.
...
|---------|--------|-------|-------|-------|
| Тестове | 25 suite, всички pass | 27 suite | 30+ suite | 40+ suite |
| Редове код | ~3,400 | ~6,000 | ~10,000 | ~15,000 |
| Поддържани БД | PostgreSQL | PG + MariaDB + SQLite | PG + MariaDB + SQLite | PG + MariaDB + SQLite |
